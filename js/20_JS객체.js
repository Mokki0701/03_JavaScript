...
btn1.addEventListener("click", ()=>{
    ...
    // 1) 객체명.key
    console.log(product.procutName, product.brand, product.price);
    ...
    // 또는 
    // 객체명["key"] = value;
    product.color = ["black", "silver", "[pink]"];
    ...
    delete product.procutName; // procutName 삭제
...
btn3.addEventListener("click", ()=>{
    ...
    const student2 = new Student("김철수", 2, 6, 1, 100);
    ...
    // 바로 studentList에 추가
    studentList.push(new Student("가로수", 1, 4, 9, 90));
    studentList.push(new Student("신짱구", 4, 3, 35, 70));

    console.log(studentList);

    // 모든 학생의 introduce() 메서드 호출
    for(let i = 0; i < studentList.length; i++){
        studentList[i].introduc();
    }
